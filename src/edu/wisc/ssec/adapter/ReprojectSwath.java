...
public class ReprojectSwath {
  ...
  public static FlatField swathToGrid(Linear2DSet grid, FlatField[] swaths, int mode) throws Exception {
     ...
     return swathsToGrid(grid, swaths, newRangeType, mode, true);
  }
  
  public static FlatField swathsToGrid(Linear2DSet grid, FlatField[] swaths, RealTupleType newRangeType, int mode, boolean filter) throws Exception {
     ...
     if (equalDom) {
         ...
         FlatField newGrid = new FlatField(new FunctionType(((SetType)grid.getType()).getDomain(), newRangeType), grid);
         FlatField[] fltFlds = new FlatField[nSwths];
         for (int k=0; k<nSwths; k++) {
             fltFlds[k] = swathToGrid(grid, swaths[k], mode, filter);
         }
         int tupDim = newRangeType.getNumberOfRealComponents();
         float[][] newRangeVals = new float[tupDim][];
         int cnt = 0;
